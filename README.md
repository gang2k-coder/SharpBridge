...
- **stopAtEntry not implemented by SharpDbg**: `debug_launch` waits briefly for an entry-point stop; SharpDbg does not send one, so launch returns `running` (honest state). `debug_continue` therefore also accepts the `Running` state: when the program is already running it **waits for the next stop instead of resuming** — the README's launch → `breakpoint_set` → `debug_continue` workflow works directly. The stop ledger delivers any stop that occurs while no tool call is waiting.

- **Capture auto-continue is race-guarded and reports failures honestly**: capture-action breakpoints auto-capture and resume on a background task. A stop-generation guard and a capture-only gate keep concurrent captures and newer stops from corrupting the session state; if a capture/auto-continue still fails (adapter error, superseded stop), the failure is **delivered to the waiting client as a truthful `stopped` event with an explanatory note** — the debuggee stays paused at the breakpoint and the next `debug_continue` resumes it, so the session never freezes silently behind a misleading `running`. Under pathological load (two adjacent capture breakpoints hit in a tight loop with concurrent tool calls) a capture can occasionally be lost, but never without a truthful stop being reported.
